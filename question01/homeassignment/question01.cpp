...
//    for (size_t i = 0; i < ITERATIONS; i++) {
//        for(size_t j = 0; j < vecOfVec[i].size(); j++)
//            mergedVec[merged_index] = vecOfVec[i][j];//directly accessing element with index is more efficiant than pushback because it doesnt check bounds or changes the size of the vector.
//        merged_index++;
//    }
//    return mergedVec;
//}
