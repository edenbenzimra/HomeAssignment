...
void dispacher_thread() {
	/*Payload* payload = new Payload(1);*///leak of memory
	auto payload = std::shared_ptr<Payload>(new Payload(1));//To fix the memory leak i decided to use the shared_ptr class that is a smart pointer that manages shared ownership of an object through pointer. it will delete the allocated memory after determining when all threads finish their tasks and will call the destructor of "payload" class to do so.
